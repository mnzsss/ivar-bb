...
describe("findHallRoot", () => {
  it("walks up to the nearest ivar.json", async () => {
    expect(await findHallRoot("/h/.ivar/repos/api/main/src", existsIn(["/h/ivar.json"]))).toBe(
      "/h",
    );
  });
...
describe("getHall", () => {
  it("reports ivar-missing when the binary cannot be spawned", async () => {
    ...
    expect(await getHall("/h", run, existsIn(["/h/ivar.json"]))).toEqual({
      status: "ivar-missing",
    });
  });
  it("combines repo list and feature list, reading promotions from the list", async () => {
    ...
    const run: Exec = async (_cmd, args) => {
      ...
      const out: Record<string, unknown> = {
        ...
        "--json feature list": {
          features: [{ name: "checkout", repos: ["api"], state: "active" }],
        },
      };
    ...
    expect(hall).toEqual({
      ...
      root: "/h",
      repos: ["api", "web"],
      features: [{ name: "checkout", promoted: ["api"] }],
    });
    expect(calls).toHaveLength(2);
  });
  it("throws the ivar error when a listing fails", async () => {
    const run: Exec = async (_c, args) =>
  ...
  it("throws when ivar output does not match the expected shape", async () => {
    ...
    await expect(getHall("/h", run, existsIn(["/h/ivar.json"]))).rejects.toThrow("expected array");
  });
});
...
describe("hallCommandArgs", () => {
  it("builds argv per command with -- before user-supplied positionals", () => {
    ...
    expect(hallCommandArgs({ command: "feature create", name: "--base=evil" })).toEqual([
      "feature",
      "create",
      "--",
      "--base=evil",
    ]);
    expect(hallCommandArgs({ command: "promote", feature: "checkout", repo: "api" })).toEqual([
      "feature",
      "promote",
      "checkout",
      "api",
  ...
  it("refuses flag-like promote values since promote cannot take --", () => {
    expect(() =>
      hallCommandArgs({ command: "promote", feature: "checkout", repo: "--base=x" }),
    ).toThrow('must not start with "-"');
  });
  it("refuses a flag-like promote feature", () => {
    expect(() => hallCommandArgs({ command: "promote", feature: "--base=x", repo: "api" })).toThrow(
      'must not start with "-"',
    );
  });
});
...
describe("runIvar", () => {
  it("runs the built argv in the hall root", async () => {
    ...
    const run: Exec = async (c, args, cwd) => {
      seen.push([c, args, cwd]);
