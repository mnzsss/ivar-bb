...
// ponytail: untracked files are not included; add `git ls-files --others --exclude-standard` + `git diff --no-index /dev/null <f>` if users need them.
  ...
  const perRepo = await Promise.all(
    status.repos
      ...
      .map(async (r) => {
        ...
        return splitPatch(r.repo, diff.stdout);
      }),
  );
  return perRepo.flat();
}
