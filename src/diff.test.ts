...
const status = (...repos: Array<{ repo: string; present?: boolean }>) => JSON.stringify({ repos: repos.map((r) => ({
  repo: r.repo, worktree: `/h/.ivar/repos/${r.repo}/checkout`, base: "main", worktree_present: r.present ?? true, state: "ready",
})) });
...
describe("splitPatch", () => {
  ...
  it("reads paths with spaces, dropping git's trailing tab", () => {
    const spaced = "diff --git a/x b/y b/x b/y\n--- a/x b/y\t\n+++ b/x b/y\t\n@@ -1 +1 @@\n-a\n+b\n";
  ...
  it("uses the old path for deletions and the header for mode-only changes", () => {
    const chunks = "diff --git a/gone.ts b/gone.ts\ndeleted file mode 100644\n--- a/gone.ts\n+++ /dev/null\n@@ -1 +0,0 @@\n-x\n"
...
describe("featureDiff", () => {
  it("diffs each present worktree against its merge-base with the base, including uncommitted changes", async () => {
    ...
    const run: Exec = async (cmd, args, cwd) => {
      ...
      if (cmd === "ivar") return { code: 0, stderr: "", stdout: status({ repo: "api" }, { repo: "web", present: false }) };
  ...
  it("reports a failing git command for that repo and keeps the other repos", async () => {
    const run: Exec = async (cmd, args, cwd) => {
      if (cmd === "ivar") return { code: 0, stderr: "", stdout: status({ repo: "api" }, { repo: "web" }) };
      if (args[0] === "merge-base") return cwd.includes("/web/")
    ...
    expect(errors).toEqual([{ repo: "web", message: expect.stringContaining("Not a valid object name") }]);
  });
  it("reports a git diff --no-index failure beyond exit 1", async () => {
    const run: Exec = async (cmd, args) => {
      ...
      if (args.includes("--no-index")) return { code: 2, stdout: "", stderr: "error: could not access" };
      return { code: 0, stdout: "", stderr: "" };
    };
    expect((await featureDiff("/h", "checkout", run)).errors).toEqual([{ repo: "api", message: expect.stringContaining("could not access") }]);
  });
