...
export function ReviewView({ projectId, feature }: { projectId: string; feature: string }) {
  ...
  const reportDiffError = (e: unknown) => setDiffError(errorMessage(e));
  const reportCommentsError = (e: unknown) => setCommentsError(errorMessage(e));
  const refreshDiff = useCallback(
    () =>
      rpc.call("feature.diff", { projectId, feature }).then((r) => {
        setFiles(r.files);
  ...
  const refreshComments = useCallback(
    () =>
      rpc.call("comments.list", { projectId, feature }).then((r) => {
  ...
  const resolve = (id: string) => rpc.call("comments.resolve", { projectId, feature, id }).then(refreshComments, reportCommentsError);
  const send = () => rpc.call("comments.send", { projectId, feature }).then((r) => setThreads(r.threads), reportCommentsError);
  ...
  return (
    <div className="flex flex-col text-sm text-[var(--foreground)]">
      ...
      <div className="flex flex-col gap-4 p-4">
        {diffError && <p role="alert" className="m-0 text-[var(--destructive-text)]">{diffError}</p>}
        {commentsError && <p role="alert" className="m-0 text-[var(--destructive-text)]">{commentsError}</p>}
        {threads.length > 0 && (
