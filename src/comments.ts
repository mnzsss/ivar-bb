...
export function buildThreadPrompt(
  ...
  return [
    ...
    ...comments.map(
      (c) =>
        `- ${c.file}:${c.line_start}-${c.line_end} — ${c.body}\n  resolve: \`${bin} review comment resolve -- ${feature} ${c.id}\``,
    ),
  ].join("\n");
}

export async function sendToThreads(
  root: string,
  feature: string,
  run: Exec,
  spawn: SpawnThread,
  ...
  for (const { repo, worktree } of status.repos) {
    ...
    const { threadId } = await spawn({
      repo,
      worktree,
      prompt: buildThreadPrompt(feature, repo, comments, ivarBin),
      title: `Review: ${feature} / ${repo}`,
    });
