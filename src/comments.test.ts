...
const open = (id: string, repo: string): ReviewComment => ({
  id,
  repo,
  file: "src/a.ts",
  line_start: 3,
...
describe("addComment", () => {
  it("passes values in --flag=value form and the feature after --", async () => {
    ...
    const run: Exec = async (_c, args) => {
      seen = args;
      return { code: 0, stderr: "", stdout: JSON.stringify(open("c1", "api")) };
    };
    const c = await addComment(
      "/h",
      "checkout",
      { repo: "api", file: "src/a.ts", lineStart: 3, lineEnd: 5, body: "- starts with a dash" },
      run,
    );
    expect(seen).toEqual([
      "--json",
  ...
  it("throws the ivar stderr on failure", async () => {
    ...
    await expect(
      addComment("/h", "x", { repo: "api", file: "a", lineStart: 1, lineEnd: 1, body: "b" }, run),
...
describe("sendToThreads", () => {
  it("spawns one thread per repo with open comments, in that repo's worktree", async () => {
    const repo = (name: string) => ({
      repo: name,
      worktree: `/h/.ivar/repos/${name}/checkout`,
      base: "main",
    ...
    const result = await sendToThreads("/h", "checkout", run, async (t) => {
      spawned.push(t);
      return { threadId: `t-${t.repo}` };
    });
    expect(spawned.map((t) => [t.repo, t.worktree])).toEqual([
      ["api", "/h/.ivar/repos/api/checkout"],
      ["web", "/h/.ivar/repos/web/checkout"],
    ]);
    expect(result).toEqual([
      { repo: "api", threadId: "t-api" },
      { repo: "web", threadId: "t-web" },
    ]);
  });
});
